...
{\normalfont\bfseries\fontsize{14}{14}\selectfont\uppercase}
{\thechapter.}
{1em}
{}
...
% Nagłówek 2.
\titleformat{\section}
{\normalfont\bfseries\itshape\fontsize{12}{12}\selectfont}
{\thesection}
{1em}
{}
...
\renewcommand{\cfttoctitlefont}{\bfseries\fontsize{12pt}{14pt}\selectfont} % Styl tytułu spisu treści
\renewcommand{\cftsecfont}{\fontsize{12pt}{14pt}\selectfont} % Rozmiar czcionki dla sekcji w spisie treści
\renewcommand{\cftloftitlefont}{\bfseries\fontsize{12pt}{14pt}\selectfont}
\renewcommand{\cftsubsecfont}{\fontsize{12pt}{14pt}\selectfont} % Rozmiar czcionki dla podsekcji w spisie treści
\renewcommand{\cftbeforetoctitleskip}{0pt} % Odstęp przed tytułem spisu treści
\renewcommand{\cftaftertoctitleskip}{12pt} % Odstęp po tytule spisu treści
\setlength{\cftbeforesecskip}{6pt} % Odstęp przed każdą sekcją
\setlength{\cftbeforesubsecskip}{6pt} % Odstęp przed każdą podsekcją
\setlength{\cftbeforechapskip}{6pt} % Odstęp przed każdym rozdziałem
...
\lstset{
    language=PHP,                % Język programowania, możesz zmienić na inny, np. C++, Java, itd.
    basicstyle=\ttfamily\footnotesize, % Podstawowy styl czcionki dla kodu
    numbers=left,                   % Numeracja linii po lewej stronie
    numberstyle=\tiny,              % Rozmiar czcionki numerów linii
    stepnumber=1,                   % Każda linia będzie numerowana
    numbersep=10pt,                 % Odstęp numerów linii od kodu
    backgroundcolor=\color{white},  % Kolor tła kodu
    showspaces=false,               % Nie pokazuj znaków spacji
    showstringspaces=false,         % Nie pokazuj spacji w łańcuchach tekstowych
    showtabs=false,                 % Nie pokazuj znaków tabulacji
    frame=single,                   % Ramka wokół kodu
    rulecolor=\color{black},        % Kolor ramki
    tabsize=2,                      % Szerokość tabulacji
    captionpos=b,                   % Pozycja podpisu (b = poniżej)
    breaklines=true,                % Zawijanie długich linii
    breakatwhitespace=false,        % Zawijaj linie tylko przy białych znakach
    keywordstyle=\color{blue},      % Styl słów kluczowych
    commentstyle=\color{green},     % Styl komentarzy
    stringstyle=\color{red},        % Styl łańcuchów tekstowych
    escapeinside={\%*}{*)},         % Możliwość ucieczki do LaTeX w kodzie
}
...
\begin{document}
    ...
    \setcounter{page}{1}
    \thispagestyle{empty}
    \includepdf{includes/pdf/strona_tytułowa_v2.pdf}
